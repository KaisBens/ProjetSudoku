public class resolveurGrille {
    ...
    public static boolean isValid2(int[][] grille, int row, int col, int num) {
        // Check the row
        for (int x = 0; x < 9; x++) {
            if (grille[row][x] == num && x != col) {
                return false;
            }
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (grille[i + startRow][j + startCol] == num && (i + startRow) != row && (j + startCol) != col) {
                    return false;
                }
            }
        }
